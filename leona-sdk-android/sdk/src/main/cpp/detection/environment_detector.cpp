...
bool contains_ci(const std::string& haystack, const char* needle) {
  return lower_copy(haystack).find(lower_copy(std::string(needle))) !=
         std::string::npos;
}
...
void add_event(EventList& out, const char* id, Severity severity,
               const char* message, const EvidenceBuilder& evidence) {
...
void check_virtualization_props(EventList& out) {
  ...
  static const char* kGuestServices[] = {
      "init.svc.nemuinit",   "init.svc.nemuinput",    "init.svc.nemu_sys_opt",
  ...
  if (service_count >= 1) {
    ...
    add_event(out, "env.emulator.runtime.guest_control_services",
  ...
  static const char* kGuestMetadataProps[] = {
      "nemud.player_package", "nemud.player_engine",
      "nemud.player_version", "nemud.player_architecture",
      "nemud.player_uuid",
  };
  ...
  if (metadata_count > 0) {
    ...
    add_event(out, "env.emulator.runtime.guest_metadata_props", Severity::HIGH,
...
void check_cpu_virtualization(EventList& out) {
  ...
  if (contains(lowered, "linux,dummy-virt")) {
    ...
    add_event(out, "env.emulator.cpu.dummy_virt_hardware", Severity::HIGH,
              "CPU hardware field exposes a virtual machine platform", ev);
  }
  ...
  if (contains(lowered, "hypervisor")) {
    ...
    add_event(out, "env.emulator.cpu.hypervisor_flag", Severity::HIGH,
              "CPU flags expose hypervisor execution", ev);
  }

  const size_t processor_count = count_occurrences(lowered, "processor");
  const size_t bogomips_48_count =
      count_occurrences(lowered, "bogomips\t: 48.00") +
      count_occurrences(lowered, "bogomips : 48.00");
  const size_t zero_part_count =
      count_occurrences(lowered, "cpu part\t: 0x000") +
      count_occurrences(lowered, "cpu part : 0x000");
  const size_t apple_impl_count =
      count_occurrences(lowered, "cpu implementer\t: 0x61") +
      count_occurrences(lowered, "cpu implementer : 0x61");
  if (processor_count >= 2 && bogomips_48_count >= processor_count &&
    ...
    add_event(out, "env.emulator.cpu.synthetic_arm_profile", Severity::MEDIUM,
              "CPU topology looks synthesized by a virtualization layer", ev);
  }
}
...
void check_virtual_mounts(EventList& out) {
  ...
  while (line_start < mounts.size()) {
    ...
    const std::string lowered_line =
        lowered_mounts.substr(line_start, end - line_start);
    const bool is_virtio_9p = lowered_line.find(" 9p ") != std::string::npos &&
  ...
  if (virtio_9p_count > 0) {
    ...
    add_event(out, "env.emulator.fs.virtio_9p_shared_mount", Severity::HIGH,
              "Host shared storage is mounted through virtio 9p", ev);
  }
  ...
  if (host_shared_count > 0) {
    ...
    add_event(out, "env.emulator.fs.host_shared_storage", Severity::MEDIUM,
              "Host-shared storage mount is visible to the guest", ev);
  }
}
...
void check_virtual_network(EventList& out) {
  ...
  if (contains(lowered, "0002000a") || contains(lowered, "0202000a")) {
    ...
    add_event(
        out, "env.emulator.net.qemu_nat_subnet", Severity::MEDIUM,
...
void check_identity_virtualization_conflict(EventList& out) {
  ...
  const bool has_virtual_runtime = contains(hv, "qemu") ||
  ...
  const std::string identity =
      lower_copy(manufacturer + " " + model + " " + fingerprint);
  static const char* kConsumerBrands[] = {
      "huawei", "honor", "xiaomi",  "redmi",  "samsung",
      "oppo",   "vivo",  "oneplus", "realme", "google",
  };
  ...
  const std::string cpu_sample =
      first_matching_line(cpuinfo, "linux,dummy-virt");
  if (!cpu_sample.empty()) ev.add("cpuSample", cpu_sample);
  add_event(out, "env.emulator.identity.consumer_brand_virtualized",
            Severity::HIGH,
            "Device identity claims a consumer handset while runtime exposes "
            "virtualization",
            ev);
}
...
void check_boot_virtualization(EventList& out) {
  ...
  const bool has_boot_marker =
      ...
      contains(lowered, "qemu") || contains(lowered, "virtio");
  if (!has_boot_marker) return;

  EvidenceBuilder ev;
  ev.add("path", "/proc/cmdline");
  ev.add("sample", sanitize_sample(cmdline));
  add_event(out, "env.emulator.kernel.virtual_boot_args", Severity::HIGH,
            "Kernel command line exposes virtual Android boot arguments", ev);
}
